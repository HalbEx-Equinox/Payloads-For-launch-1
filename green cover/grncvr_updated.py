...
checker= input(" enter the address and name of the text file : ex:-PycharmProjects/myprojects/hmm/")
checker1= input(" enter the address and name of the image file : ex:-PycharmProjects/myprojects/tri/")
...
while(1):        
    ...
    file1=open(checker+str(lala)+".txt","w")
    cv2.imshow('This is whats seen',frame) 
    cv2.imshow('mask',mask) 
    #cv2.imshow('res',res) 
    #cv2.imshow('res',hsv1)
    cv2.imwrite(checker1+str(lala)+".jpg", frame)
    #print('green cover percentage:', np.round(g/r*100, 2))
    cvti=np.round(g/r*100, 2)
    L=['green cover percentage:', cvti,'\n']
